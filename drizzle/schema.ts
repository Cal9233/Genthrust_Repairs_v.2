import { mysqlTable, mysqlSchema, AnyMySqlColumn, foreignKey, primaryKey, varchar, text, int, bigint, double, timestamp, unique, datetime, index, json, tinyint } from "drizzle-orm/mysql-core"
...
export const active = mysqlTable("active", {
	...
	trackingNumberPickingUp: varchar("TRACKING_NUMBER_PICKING_UP", { length: 500 }),
...
export const binsInventoryActual = mysqlTable("bins_inventory_actual", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const hold = mysqlTable("hold", {
...
export const inventoryindex = mysqlTable("inventoryindex", {
	...
	lastSeen: datetime("LastSeen", { mode: 'string'}).default(sql`(CURRENT_TIMESTAMP)`),
},
(table) => [
...
export const net = mysqlTable("net", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	ro: double("RO"),
...
export const pnNoReparadas727 = mysqlTable("pn_no_reparadas_727", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
...
export const pnNoReparadasMd82 = mysqlTable("pn_no_reparadas_md82", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	partNo: varchar("PART_No", { length: 500 }),
	serial: varchar("SERIAL", { length: 500 }),
	description: varchar("DESCRIPTION", { length: 500 }),
...
export const terra = mysqlTable("terra", {
	id: bigint({ mode: "number" }).autoincrement().notNull(),
	partNo: varchar("PART_No", { length: 500 }),
	serial: varchar("SERIAL", { length: 500 }),
	description: varchar("DESCRIPTION", { length: 500 }),
